...
public class ctyxzrr956 {
    ...
    public static void showErrorDialog(Activity activity, BVM baseViewModel) {
        if(null == popupView)
         popupView = new XPopup.Builder(activity)
                ...
                .asConfirm("", netTip()[0],
                        "",netTip()[1],
                        () -> RHelp.reconnect(true, baseViewModel), null, true);
    ...
    public static int getPlatform() {
        int platform = mdhaat979.game_ca_beo66_INT_CHANNEL_ID;
        String channelId = wovg23.instance().popString(mdhaat979.game_ca_beo66_CHANNEL_ID);
        if (!channelId.isEmpty()) {
            try {
                platform = Integer.parseInt(channelId);
            } catch (Exception e) {
                platform = mdhaat979.game_ca_beo66_INT_CHANNEL_ID;
            }
        }
        return platform;
    }
