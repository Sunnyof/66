...
//		tvAboutAppInfo.setText(BaseApplication.getInstance().getPackageName()
//				+ "\n" + BaseApplication.getInstance().getPackageName());
	...
	private void downloadApp() {
		...
		runThread(TAG + "downloadApp", new Runnable() {
			@Override
			public void run() {
				File file = DownloadUtil.downLoadFile(context, "ZBLibraryDemo", ".apk", "11");
	...
	public void onDragBottom(boolean rightToLeft) {
		if (rightToLeft) {
			toActivity(WebViewActivity.createIntent(context, "博客", "22"));
	...
	public void onClick(View v) {
		...
		case R.id.llAboutUpdate:
			startActivity(SettingActivity.createIntent(context));
			overridePendingTransition(R.anim.bottom_push_in, R.anim.hold);
