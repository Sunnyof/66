...
const importMapJson = jsb.fileUtils.getStringFromFile("src/import-map.74628.json");
const importMap = JSON.parse(importMapJson);
System.warmup({
    importMap,
    importMapUrl: 'src/import-map.74628.json',
    defaultHandler: (urlNoSchema) => {
        require(urlNoSchema.startsWith('/') ? urlNoSchema.substr(1) : urlNoSchema);
    },
});

System.import('./src/application.0804e.js')
.then(({ Application }) => {
