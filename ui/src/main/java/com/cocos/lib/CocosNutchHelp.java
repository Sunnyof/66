...
class CocosNutchHelp {
        ...
        if (hasNotch(activity)) {
            ...
            if (manufacturer.equalsIgnoreCase("xiaomi")) {
                ...
                notchHeight = (long) TypedValue.applyDimension(
                        ...
                        32f,
                        BaseApplication.application.getApplicationContext().getResources().getDisplayMetrics()
                );
    ...
    private int getInt(String key) {
        ...
        if (Build.MANUFACTURER.equalsIgnoreCase("Xiaomi")) {
            try {
                ClassLoader classLoader = BaseApplication.application.getClassLoader();
    ...
    private boolean hasNotchAtOPPO() {
        return BaseApplication.application.getPackageManager()
    ...
    private boolean hasNotchSamsung() {
        if (Build.MANUFACTURER.equalsIgnoreCase("samsung")) {
            try {
                Resources res = BaseApplication.application.getResources();
                int resId =
    ...
    private int getNotchSizeAtHuaWei() {
        int height = 0;
        try {
            ClassLoader cl = BaseApplication.application.getClassLoader();
    ...
    public static CocosNutchHelp getInstance() {
        return new CocosNutchHelp();
    }
